...
export async function getLatestEvents(limit=24){
  ...
  return rows.map((r:any)=>({...r,date:r.event_date?.toISOString?.().slice(0,10)??String(r.event_date)}));
}
...
export async function getTopic(slug:string){
  ...
  return {...r[0],events:ev.map((e:any)=>({...e,date:e.event_date?.toISOString?.().slice(0,10)??String(e.event_date)}))};
}
