...
const q2dEvent = {
  ...
  date:'2026-09-08',
  title:'Perplexity Q2D-Web maps the retrieval stage behind agentic search',
  summary:'Perplexity released Q2D-Web, a production-shaped retrieval benchmark built around approximately 190 million web documents and 69,721 machine-reformulated queries across 10 languages.',
  what_changed:'The benchmark evaluates the machine-generated search queries an AI agent produces after receiving a user prompt, instead of testing only human-written queries. It also separates retrieval relevance from later citation behavior.',
  why_it_matters:'The prompt a user types is not necessarily the query your content competes for. AI visibility must account for query reformulation, first-stage retrieval, reranking, evidence selection, and citation as distinct stages.',
  interpretation:'Visibility OS treats Q2D-Web as strong evidence for a multi-stage visibility model: retrievable does not mean selected, selected does not mean cited, and cited does not necessarily mean mentioned prominently.',
  ...
  significance:'major',
  topics:['retrieval','query-fan-out','citations','geo','llmo','measurement'],
  source:'perplexity-q2d-web-2026',
  sources:[
    {slug:'perplexity-q2d-web-2026',title:'Q2D-Web',publisher:'Perplexity',publisher_text:'Perplexity',source_type:'official',published_at:'2026-09-08',url:'https://www.perplexity.ai/hub/blog/q2d-web'},
    {slug:'q2d-web-paper-2026',title:'Q2D-Web: A Production-Shaped Benchmark for First-Stage Retrieval in Agentic Search',publisher:'arXiv',publisher_text:'arXiv',source_type:'research',published_at:'2026-09-08',url:'https://arxiv.org/abs/2609.08887'}
  ]
};
...
export async function getLatestEvents(limit=24){
  ...
  const rows=await sql`select e.* from public.events e where e.status='published' order by e.event_date desc,e.updated_at desc limit ${Math.max(limit-1,1)}`;
  const dbEvents=rows.map((r:any)=>({...r,date:r.event_date?.toISOString?.().slice(0,10)??String(r.event_date)}));
  const merged=[q2dEvent,...dbEvents.filter((e:any)=>e.slug!==q2dEvent.slug)].sort((a:any,b:any)=>String(b.date).localeCompare(String(a.date)));
  return merged.slice(0,limit);
}

export async function getEvent(slug:string){
  if(slug===q2dEvent.slug) return q2dEvent;
  if(!sql) return fallbackEvents.find(e=>e.slug===slug)??null;
  const rows=await sql`select * from public.events where slug=${slug} and status='published' limit 1`; if(!rows[0]) return null;
  const r:any=rows[0];
  const topics=await sql`select t.slug from public.event_topics et join public.topics t on t.id=et.topic_id where et.event_id=${r.id}`;
  ...
  return {...r,date:r.event_date?.toISOString?.().slice(0,10)??String(r.event_date),topics:topics.map((x:any)=>x.slug),sources};
}
...
export async function getTopic(slug:string){
  ...
  const dbEvents=ev.map((e:any)=>({...e,date:e.event_date?.toISOString?.().slice(0,10)??String(e.event_date)}));
  const extra=q2dEvent.topics.includes(slug)?[q2dEvent]:[];
  return {...r[0],events:[...extra,...dbEvents.filter((e:any)=>e.slug!==q2dEvent.slug)].sort((a:any,b:any)=>String(b.date).localeCompare(String(a.date)))};
}
...
export async function getSources(){
  ...
  const db=await sql`select * from public.sources order by published_at desc nulls last`;
  return [...q2dEvent.sources,...db.filter((s:any)=>!['perplexity-q2d-web-2026','q2d-web-paper-2026'].includes(s.slug))];
}
...
export async function searchEvents(q:string,limit=20){
  ...
  const extra=Object.values(q2dEvent).flat().join(' ').toLowerCase().includes(n)?[q2dEvent]:[];
  if(!sql) return fallbackEvents.filter(e=>Object.values(e).flat().join(' ').toLowerCase().includes(n)).slice(0,limit);
  const rows=await sql`select * from public.search_published_events(${q},${limit})`;
  const db=rows.map((r:any)=>({...r,date:r.event_date?.toISOString?.().slice(0,10)??String(r.event_date)}));
  return [...extra,...db.filter((e:any)=>e.slug!==q2dEvent.slug)].slice(0,limit);
}
